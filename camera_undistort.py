...
@click.option('--crop/--no-crop', ' /-c', default = False, help='Use ROI to crop undistorted image')
def main(input_image, yaml_file, crop):
    ...
    cv2.imshow('distorted-undistorted', cv2.resize(stack_img, (0,0), fx = 0.5, fy = 0.5, interpolation= 1))
    cv2.waitKey(0)
